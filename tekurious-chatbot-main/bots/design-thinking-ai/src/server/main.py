...
log = logging.getLogger("design-thinking-ai.chat")
...
def _compact_education_response(text: str, max_chars: int = 700, max_sentences: int = 5) -> str:
    """Keep education answers concise and readable for UI responses."""
    ...
    truncated = short[:max_chars].rsplit(" ", 1)[0].strip()
    return f"{truncated}..."
...
register_playground(app, "Design Thinking (standalone)")
...
async def chat(request: Request, accept: str | None = Header(default=None)):
    ...
    try:
        ...
        if not guardrails_output or guardrails_output.strip().upper() != "YES":
            refusal = "Sorry, I can only help with design thinking and innovation topics."
        ...
        domain_label = "design thinking and innovation"
        ...
        if _wants_event_stream(accept):
            ...
            async def event_stream():
                try:
                    ...
                    yield _sse(
                        "done",
                        {"ok": False, "request_id": request_id, "detail": str(stream_exc)},
                    )
