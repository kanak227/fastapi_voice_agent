...
log = logging.getLogger("wellbeing-ai.chat")
...
register_playground(app, "Well-being (standalone)")
...
async def chat(request: Request, accept: str | None = Header(default=None)):
    ...
    try:
        ...
        if not guardrails_output or guardrails_output.strip().upper() != "YES":
            refusal = "Sorry, I can only help with well-being and healthy habits topics."
        ...
        domain_label = "well-being and healthy habits"
