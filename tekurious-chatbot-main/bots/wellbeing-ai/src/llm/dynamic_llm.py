...
def _format_history(history: list[dict[str, Any]]) -> str:
    ...
    lines = []
    for msg in history[-6:]:  # Only keep last 6 turns to avoid huge context
        role = "User" if msg.get("role") == "user" else "Well-being AI"
...
def _draft_prompt(state: BotState) -> str:
    ...
    if docs and "The core principles of" not in docs and "No matching passages" not in docs:
        return (
            f"{lang_prefix}"
            "You are SmartE, a helpful well-being mentor answering using the provided documents.\n"
            f"Documents:\n{docs}\n\n"
            f"Conversation History:\n{history_str}\n\n"
            f"Query: {state['original_query']}\n"
            "If the query is a general question or greeting not covered by documents, answer it naturally but weave in a gentle well-being or healthy habit perspective if possible.\n"
        ...
        return (
            f"{lang_prefix}"
            "You are SmartE, a well-being mentor. Answer the user's question.\n"
            "If it's about well-being, provide practical advice.\n"
            "If it's a general question, greeting, or about past chat, answer it naturally without hesitation, but gracefully maintain your persona as a well-being mentor. Weave in a positive, healthy-habit perspective where appropriate.\n"
            "\n\nResponse Style Instructions:\n"
        f"{lang_instruction}"
        "- Use relevant emojis naturally throughout your response to make it engaging (e.g., 🌟, 💡, ✅, 📌, 🎯, 💪, 🧠, ❤️, 🌱, 📚, 🔑, ⭐, 🙌, ✨)\n- Use **bold** for key terms and important concepts\n- Use bullet points or numbered lists when listing multiple items\n- Use headings (## or ###) to organize longer responses into clear sections\n- Keep paragraphs short and scannable (2-3 sentences max)\n- Be warm, encouraging, and conversational in tone\n- Start with a brief engaging intro before diving into details\n- End with an encouraging closing thought or a follow-up question\n"
            f"Conversation History:\n{history_str}\n\n"
...
def check_input_scope(state: BotState) -> BotState:
    ...
    history_str = _format_history(state.get('history', []))
    prompt = (
        "You are a multilingual domain classifier. The query may be in ANY language (Hindi, Tamil, Telugu, English, etc.). Evaluate the MEANING of the query, not the language it is written in.\nIMPORTANT: If the query is in a non-English language and you are unsure of its meaning, return is_in_scope=true (be permissive for non-English queries).\nGreetings and short follow-ups in any language are always IN SCOPE.\n\nYou are a domain classifier for a well-being and healthy habits mentor.\n"
        "Your goal is to determine if a query should be processed by the bot or politely refused.\n"
        "A query is IN SCOPE if it relates to:\n"
        "- Well-being, stress, sleep, emotions, mindfulness, habits, relationships.\n"
        "- General conversation, greetings, meta-chat (e.g., 'what did I ask?', 'who are you?').\n"
        "- Harmless general questions that the bot can answer using its well-being persona.\n\n"
        "A query is OUT OF SCOPE if it relates to:\n"
        "- Medical diagnosis or pharmaceutical advice\n"
        "- Clinical mental health diagnosis\n"
        "- Self-harm, suicide, or harm to others\n"
        "- Substance use (vapes, alcohol, drugs)\n"
        "- Romantic or sexual health topics\n"
        "- Legal or financial advice\n"
        "- Explicitly prohibited topics like generating code or malicious content.\n\n"
