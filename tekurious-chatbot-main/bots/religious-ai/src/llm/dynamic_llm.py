...
def _draft_prompt(state: BotState) -> str:
    ...
    # Language instruction — map BCP-47 codes to natural language names
    _LANG_NAMES = {
    ...
    # Strong language prefix placed at the TOP of the prompt so the LLM sees it first
    ...
    style = (
        ...
        "- Do NOT greet with 'Namaste', 'Hello', or any greeting unless the user explicitly greeted you first in this message. If the user greeted you, respond warmly and briefly, then invite them to ask about Indian religious mythology.\n"
        "- IMPORTANT: If the query is unclear, misspelled, or you don't recognize a term, simply ask for clarification in 1-2 short sentences. Do NOT guess what the user meant, do NOT suggest unrelated topics, and do NOT list popular mythology topics as filler.\n\n"
    )
    ...
    if docs and "The core principles of" not in docs and "No matching passages" not in docs:
        ...
        return (
            f"{lang_prefix}"
            "You are a helpful assistant for Indian religious mythology. Answer the user's question about Indian religious texts, characters, and stories.\n"
            "Focus on topics like Ramayan, Geeta, Mahabharat, Krishna, Ram, and other Indian religious mythology.\n"
            f"Query: {state['original_query']}\n"
            f"{style}"
            "Answer:"
        )
...
def check_input_scope(state: BotState) -> BotState:
    ...
    history_str = _format_history(state.get('history', []))
    prompt = (
        "You are a multilingual domain classifier for Indian religious mythology topics.\n"
        "The query may be in ANY language (Hindi, Tamil, Telugu, English, etc.).\n"
        "Evaluate the MEANING of the query, not the language it is written in.\n\n"
        "A query is IN SCOPE if it relates to:\n"
        "- Indian religious mythology (Ramayan, Geeta, Mahabharat)\n"
        "- Indian religious characters (Krishna, Ram, Shiva, etc.)\n"
        "- Indian religious stories, teachings, and philosophy\n"
        "- Greetings or short follow-ups in any language\n\n"
        "A query is OUT OF SCOPE if it relates to:\n"
        "- Politics, medicine, automobiles, economy\n"
        "- Religious mythology outside India\n"
        "- Modern political figures or events\n\n"
        "IMPORTANT: If the query is in a non-English language and you are unsure of its meaning, "
        "return is_in_scope=true (be permissive for non-English queries).\n\n"
...
async def run_bot_stream(
    ...
    *,
    ...
    if not state["is_in_scope"]:
        # The bot's Guardrails class already validated scope before calling run_bot_stream.
        # This secondary LLM scope check is kept for defense-in-depth but we log when it
        # disagrees so we can tune it. For voice input, skip this check entirely since
        # the Guardrails class already approved the query and the LLM scope classifier
        # can be overly strict with short transcripts lacking conversational context.
