...
export default function Dashboard() {
  ...
  const voiceSessionIdRef = useRef(`financial-literacy-session-${Date.now()}`);

  // Initialize messages on client side only to prevent hydration mismatch
  useEffect(() => {
    setIsClient(true);
    setMessages([
      { id: 1, content: "Hello! I'm your Financial Literacy guide. Let's learn about money, budgeting, and smart financial habits!", sender: "bot", time: formatTime(new Date()) },
    ]);
  }, []);
...
const sendMessageToAPI = async (userMessage) => {
  try {
    ...
    const response = await fetch("/api/FinancialLiteracy", {
      method: "POST",
      headers: {
  ...
  const clearChat = () => {
    setHandsFreeVoiceEnabled(false);
    stopListening();
    voiceSessionIdRef.current = `financial-literacy-session-${Date.now()}`;
    setVoicePhase('idle');
    setVoiceError('');
    setMessages([
      { id: 1, content: "Hello! I'm your Financial Literacy guide. Let's learn about money, budgeting, and smart financial habits!", sender: "bot", time: formatTime(new Date()) },
        ...
        <div className="flex flex-col flex-1 min-w-0">
          <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 shadow-sm">
            <div className="flex items-center space-x-4">
              <h1 className="text-xl font-semibold text-gray-800">Financial Literacy AI</h1>
            </div>
    ...
    <div className="flex h-screen w-full bg-zinc-100">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 shrink-0 items-center justify-between border-b border-zinc-200/80 bg-white px-4">
          <h1 className="text-[15px] font-semibold tracking-tight text-zinc-900">Financial Literacy AI</h1>
          <Avatar className="h-8 w-8">
