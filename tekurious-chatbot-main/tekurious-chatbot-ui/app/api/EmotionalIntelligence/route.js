...
import {
  EMOTIONAL_INTELLIGENCE_FALLBACK,
  isEmotionalIntelligenceTopicAllowedByIntent,
...
const DOMAIN_SLUG = "emotional-intelligence";
...
export async function POST(request) {
  ...
  const sid = session_id || `emotional-intelligence-${Date.now()}`;

  if (!isEmotionalIntelligenceTopicAllowedByIntent(normalizedQuery)) {
    return NextResponse.json({ response: EMOTIONAL_INTELLIGENCE_FALLBACK });
  }
  ...
  return NextResponse.json({
    response: responseText || EMOTIONAL_INTELLIGENCE_FALLBACK,
  });
}
