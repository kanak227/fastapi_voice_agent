...
import {
  FINANCIAL_LITERACY_FALLBACK,
  isFinancialLiteracyTopicAllowedByIntent,
...
const DOMAIN_SLUG = "financial-literacy";
...
export async function POST(request) {
  ...
  const sid = session_id || `financial-literacy-${Date.now()}`;

  if (!isFinancialLiteracyTopicAllowedByIntent(normalizedQuery)) {
    return NextResponse.json({ response: FINANCIAL_LITERACY_FALLBACK });
  }
  ...
  return NextResponse.json({
    response: responseText || FINANCIAL_LITERACY_FALLBACK,
  });
}
