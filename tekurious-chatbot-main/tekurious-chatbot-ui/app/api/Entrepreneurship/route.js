...
import {
  ENTREPRENEURSHIP_FALLBACK,
  isEntrepreneurshipTopicAllowedByIntent,
...
export async function POST(request) {
  ...
  const sid = session_id || `entrepreneurship-${Date.now()}`;

  if (!isEntrepreneurshipTopicAllowedByIntent(normalizedQuery)) {
    return NextResponse.json({ response: ENTREPRENEURSHIP_FALLBACK });
  }
  ...
  return NextResponse.json({
    response: responseText || ENTREPRENEURSHIP_FALLBACK,
  });
}
