...
import {
  ...
  isSustainabilityTopicAllowedByIntent,
...
const DOMAIN_SLUG = "sustainability";
...
export async function POST(request) {
  ...
  const sid = session_id || `sustainability-${Date.now()}`;

  if (!isSustainabilityTopicAllowedByIntent(normalizedQuery)) {
    return NextResponse.json({ response: SUSTAINABILITY_FALLBACK });
  }
  ...
  return NextResponse.json({
    response: responseText || SUSTAINABILITY_FALLBACK,
  });
}
