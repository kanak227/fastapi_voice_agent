...
import {
  DESIGN_THINKING_FALLBACK,
  isDesignThinkingTopicAllowedByIntent,
...
const DOMAIN_SLUG = "design-thinking";
...
export async function POST(request) {
  ...
  const sid = session_id || `design-thinking-${Date.now()}`;

  if (!isDesignThinkingTopicAllowedByIntent(normalizedQuery)) {
    return NextResponse.json({ response: DESIGN_THINKING_FALLBACK });
  }
  ...
  return NextResponse.json({
    response: responseText || DESIGN_THINKING_FALLBACK,
  });
}
