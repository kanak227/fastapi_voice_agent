...
import {
  DIGITAL_LITERACY_FALLBACK,
  isDigitalLiteracyTopicAllowedByIntent,
...
const DOMAIN_SLUG = "digital-literacy";
...
export async function POST(request) {
  ...
  const sid = session_id || `digital-literacy-${Date.now()}`;

  if (!isDigitalLiteracyTopicAllowedByIntent(normalizedQuery)) {
    return NextResponse.json({ response: DIGITAL_LITERACY_FALLBACK });
  }
  ...
  return NextResponse.json({
    response: responseText || DIGITAL_LITERACY_FALLBACK,
  });
}
