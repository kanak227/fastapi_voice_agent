...
import {
  WELLBEING_FALLBACK,
  isWellbeingTopicAllowedByIntent,
...
export async function POST(request) {
  ...
  const sid = session_id || `wellbeing-${Date.now()}`;

  if (!isWellbeingTopicAllowedByIntent(normalizedQuery)) {
    return NextResponse.json({ response: WELLBEING_FALLBACK });
  }
  ...
  return NextResponse.json({
    response: responseText || WELLBEING_FALLBACK,
  });
}
